export default function validatePassword(password: string) {
    if (!password.match(/^[a-zA-Z0-9!@#._$%^&*]*$/)) {
        return 'Пароль должен состоять только из букв латиницы верхнего или нижнего регистра, цифр, точки, нижнего подчёркивания и специальных символов(!@#$%^)';
    }
    if (password.match(/\s/)) {
