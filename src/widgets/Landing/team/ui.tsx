...
export const Team: FC = () => {
    ...
    const handleArrowClick = (type: 'left' | 'right') => {
        const activeElementId = data.findIndex((element: sliderDataType) => element.isActive);
        const updatedData = data.map((element: sliderDataType, index: number) => {
            const isActive =
                (type === 'left' && index === activeElementId - 1) ||
                (type === 'right' && index === activeElementId + 1) ||
                (type === 'left' && index === data.length - 1 && activeElementId === 0) ||
                (type === 'right' && index === 0 && activeElementId === data.length - 1);
            return {
                ...element,
                isActive,
            };
